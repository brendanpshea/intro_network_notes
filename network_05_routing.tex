        ...
        \begin{column}{0.48\textwidth}
            \begin{block}{Routing Concepts}
                \begin{itemize}\setlength{\itemsep}{1pt}
                    \item \keyterm{Routing Table} -- Known destinations
                    \item \keyterm{Default Route} -- Path when no match
                    \item \keyterm{Hop} -- Each router along path
                    \item \keyterm{Metric} -- Cost to choose best route
                \end{itemize}
            \end{block}
            
            \begin{block}{Routing Protocols}
                \begin{itemize}\setlength{\itemsep}{1pt}
                    \item \keyterm{RIP} -- Simple, hop-count based
                    \item \keyterm{OSPF} -- Open standard, link-state
                    \item \keyterm{EIGRP} -- Cisco enhanced protocol
                    \item \keyterm{BGP} -- Internet backbone routing
                \end{itemize}
            \end{block}
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{block}{Network Services}
                \begin{itemize}\setlength{\itemsep}{1pt}
                    \item \keyterm{NAT} -- Private to public IP translation
                    \item \keyterm{PAT} -- Many hosts, one public IP
                    \item \keyterm{Firewall} -- Filters traffic
                    \item \keyterm{Edge Router} -- Gateway to outside
                \end{itemize}
            \end{block}
            
            \begin{block}{VLAN Concepts}
                \begin{itemize}\setlength{\itemsep}{1pt}
                    \item \keyterm{VLAN} -- Virtual LAN segmentation
                    \item \keyterm{Trunk} -- Carries multiple VLANs
                    \item \keyterm{802.1Q} -- VLAN tagging standard
                    \item \keyterm{Native VLAN} -- Untagged traffic
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
...
% =============================================================================
\begin{frame}{Case Study Solution: Miracle Max's Potion Shop}
...
% =============================================================================
% SLIDE 35: Module Summary (Part 1)
% =============================================================================
\begin{frame}{Module 5 Summary (Part 1)}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Routing Fundamentals}
                \small
                \begin{itemize}\setlength{\itemsep}{2pt}
                    \item Routers connect networks at Layer 3
                    \item \textbf{Routing tables} store known destinations
                    \item \textbf{Longest prefix match} selects best route
                    \item \textbf{Static routes}: Manual, predictable
                    \item \textbf{Default route}: Gateway of last resort
                \end{itemize}
            \end{block}
            
            \vspace{0.2em}
            \begin{block}{Dynamic Routing}
                \small
                \begin{itemize}\setlength{\itemsep}{2pt}
                    \item \textbf{RIP}: Simple, hop count (max 15)
                    \item \textbf{EIGRP}: Cisco, fast convergence
                    \item \textbf{OSPF}: Open standard, link-state
                    \item \textbf{BGP}: Internet backbone routing
                    \item \textbf{AD} decides between protocols
                \end{itemize}
            \end{block}
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{block}{NAT and PAT}
                \small
                \begin{itemize}\setlength{\itemsep}{2pt}
                    \item \textbf{NAT} translates private to public IPs
                    \item \textbf{Static NAT}: 1:1 mapping
                    \item \textbf{Dynamic NAT}: Pool-based
                    \item \textbf{PAT}: Many-to-one using ports
                    \item PAT is most common (home routers)
                \end{itemize}
            \end{block}
            ...
            \begin{block}{Firewalls}
                ...
                \begin{itemize}\setlength{\itemsep}{2pt}
                    \item Filter traffic based on rules
                    \item Types: Packet, Stateful, App, NGFW
                    \item \textbf{DMZ}: Screened subnet for servers
                    \item Defense in depth = multiple layers
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
...
% =============================================================================
% SLIDE 36: Module Summary (Part 2)
% =============================================================================
\begin{frame}{Module 5 Summary (Part 2)}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{VLANs}
                \small
                \begin{itemize}\setlength{\itemsep}{2pt}
                    \item Logically segment one switch
                    \item Create separate broadcast domains
                    \item Benefits: Security, performance, flexibility
                    \item \textbf{802.1Q} tags frames with VLAN ID
                    \item \textbf{Trunk ports} carry multiple VLANs
                    \item \textbf{Native VLAN}: Untagged traffic
                \end{itemize}
            \end{block}
            ...
            \begin{block}{Voice VLANs}
                \small
                \begin{itemize}\setlength{\itemsep}{2pt}
                    \item Separate voice from data traffic
                    \item QoS priority for phone calls
                    \item One port, two VLANs (data + voice)
                \end{itemize}
            \end{block}
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{block}{Key Takeaways}
                ...
                \begin{enumerate}\setlength{\itemsep}{3pt}
                    \item Routers make forwarding decisions based on \textbf{destination IP} and routing table
                    \item \textbf{Administrative Distance} determines protocol trustworthiness
                    \item \textbf{NAT/PAT} conserve public IP addresses
                    \item \textbf{Firewalls} protect network boundaries
                    \item \textbf{VLANs} provide logical segmentation
                    \item \textbf{Inter-VLAN routing} requires Layer 3
                \end{enumerate}
            \end{block}
