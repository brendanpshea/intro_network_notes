    ...
    \begin{columns}[T]
        \begin{column}{0.52\textwidth}
            ...
            \begin{exampleblock}{Everyday Example}
                Loading a website: home LAN $\rightarrow$ ISP WAN $\rightarrow$ internet $\rightarrow$ server. You own the LAN; the ISP owns the WAN.
            \end{exampleblock}
        \end{column}
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{FTTC --- Fiber to the Curb / Node}
                \begin{itemize}
                    \footnotesize
                    \item Fiber reaches a nearby street cabinet.
                    \item The final 100--300 meters use \textbf{copper}.
                    \item Cheaper to deploy because fiber stops short of the building.
                    \item The copper tail limits speed, especially as distance grows.
                \end{itemize}
            \end{block}
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{block}{FTTP / FTTH --- Fiber to the Premises / Home}
                \begin{itemize}
                    \footnotesize
                    \item \textbf{Pure fiber} runs all the way to the home.
                    \item An \keyterm{ONT} converts light to Ethernet indoors.
                    \item Delivers the best speed and latency because there is no copper bottleneck.
                    \item Costs more because the ISP must run fiber to each building.
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
...
\begin{frame}[t]{Wireless WAN: Cellular and Satellite}
    ...
    \begin{exampleblock}{Real-World Use: Automatic Failover}
        \footnotesize
        Many businesses use cellular WAN as a \textbf{secondary / backup} link. If the primary circuit fails, the router switches to LTE automatically --- called \textbf{WAN failover}.
    \end{exampleblock}
\end{frame}
...
\begin{frame}[t]{Case Study: Tails Sets Up the Freedom Fighter Network}
    \begin{casestudy}{Tails Sets Up the Freedom Fighter Network}
        \footnotesize
        Tails must choose WAN connectivity for three outposts, each needing \textbf{50 Mbps} minimum.
        \vspace{0.1em}
        \begin{center}
        ...
        \vspace{0.3em}
        Tails also notices: Gamma has a \textbf{1.2-second audio delay} on video calls; Beta drops to \textbf{30 Mbps} during peak hours.
    \end{casestudy}
...
\begin{frame}[t]{What Is a VPN?}
    \begin{columns}[T]
        \begin{column}{0.50\textwidth}
            \begin{itemize}
                \item The public internet is \textbf{not private}.
                \item A \keyterm{VPN} creates a secure path \textit{through} that untrusted network.
                \item Every VPN depends on three ideas:
                    \begin{enumerate}
                        \footnotesize
        ...
        \begin{column}{0.48\textwidth}
            \begin{itemize}
                ...
                \item \textbf{Two operating modes:}
                    \begin{itemize}
                        \footnotesize
                        \item \keyterm{Transport mode}: encrypts only the payload; IP header stays visible. Host-to-host.
                        \item \keyterm{Tunnel mode}: encrypts the entire packet, adds new outer IP header. Used in VPN gateways.
                    \end{itemize}
                \item \textbf{Two sub-protocols:}
                    \begin{itemize}
                        \footnotesize
                        \item \keyterm{AH}: integrity and authentication --- \textbf{no encryption}.
                        \item \keyterm{ESP}: integrity, authentication, and \textbf{encryption}. Standard VPN choice.
                    \end{itemize}
            \end{itemize}
        \end{column}
        \begin{column}{0.49\textwidth}
            \begin{center}
                \begin{tikzpicture}[scale=0.72, xshift=0.2cm]
                    % AH row
...
\begin{frame}[t]{Internet Key Exchange (IKE)}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{itemize}
                \item Before IPsec works, both peers must agree on algorithms and keys.
                \item \keyterm{IKE (Internet Key Exchange)} automates that negotiation.
                \item The result is stored in a \keyterm{Security Association (SA)}.
            \end{itemize}
            \vspace{0.3em}
            \begin{block}{Phase 1 --- Authenticate}
                \footnotesize
                Peers authenticate with a \textbf{PSK} or \textbf{certificate} and create a secure channel.
            \end{block}
            \vspace{0.2em}
            \begin{block}{Phase 2 --- Negotiate Session Keys}
                \footnotesize
                Inside that secure channel, peers choose IPsec settings and generate the session keys for data traffic.
            \end{block}
        \end{column}
        \begin{column}{0.49\textwidth}
            \begin{center}
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{itemize}
                ...
                \item A user runs \textbf{VPN client software} and connects to a corporate gateway over the internet.
                \item The gateway assigns a \textbf{virtual IP address}, so the device behaves like it is on the office network.
            \end{itemize}
            \vspace{0.3em}
            \begin{block}{Full Tunnel vs.\ Split Tunnel}
                \begin{itemize}
                    \footnotesize
                    \item \keyterm{Full tunnel}: all traffic uses the VPN. More secure, but heavier on corporate bandwidth.
                    \item \keyterm{Split tunnel}: only company traffic uses the VPN. More efficient, but less isolated.
                \end{itemize}
            \end{block}
        \end{column}
        \begin{column}{0.49\textwidth}
            \begin{center}
                \begin{tikzpicture}[scale=0.73, xshift=0.5cm]
                    % Home laptop
...
\begin{frame}[t]{Clientless VPNs (SSL VPN)}
    \begin{columns}[T]
        \begin{column}{0.50\textwidth}
            \begin{itemize}
                \item A \keyterm{clientless VPN} uses a \textbf{web browser} instead of VPN software.
                \item The user visits a secure portal, logs in, and the gateway \textbf{proxies} access to internal web resources.
                \item Best for contractors, guests, or short-term access from unmanaged devices.
            \end{itemize}
            \vspace{0.2em}
            \begin{alertblock}{Limitation}
                \footnotesize It mainly works for \textbf{web-based resources}; it does not replace a full VPN for RDP or file shares.
            \end{alertblock}
        \end{column}
        \begin{column}{0.47\textwidth}
            \begin{center}
...
\begin{frame}[t]{Site-to-Site VPNs}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{itemize}
                \item A \keyterm{site-to-site VPN} connects \textbf{two networks}, not two individual users.
                \item The tunnel is built between \textbf{routers or firewalls}.
                \item End users need no VPN software because the connection is transparent.
                \item Common use: branch-to-HQ or data-center links.
            \end{itemize}
            \vspace{0.3em}
            \begin{block}{What Users Experience}
                \footnotesize Users just connect to remote resources normally; the gateways handle the VPN in the background.
            \end{block}
        \end{column}
        \begin{column}{0.49\textwidth}
            \begin{center}
                \begin{tikzpicture}[scale=0.68, xshift=0.48cm]
                    % HQ site
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{alertblock}{Network+ Exam Tip}
                \footnotesize Ask three questions: one user, browser-only access, or two full office networks?
            \end{alertblock}
        \end{column}
        \begin{column}{0.48\textwidth}
...
\begin{frame}[t]{Case Study: Shadow and G.U.N.\ Remote Access}
    \begin{casestudy}{Shadow and G.U.N.\ Remote Access}
        \footnotesize
        Shadow must solve three remote-access problems for G.U.N.:
        ...
        \begin{center}
            \begin{tabular}{l p{7.8cm}}
                ...
                \midrule
                A & Three field offices need always-on access to HQ file servers through managed routers/firewalls. \\
                B & Agents on personal laptops need full access from untrusted networks. \\
                C & A contractor needs browser-only access to one internal web portal with no install. \\
                \bottomrule
            \end{tabular}
        \end{center}
    \end{casestudy}
...
\begin{frame}[t]{Case Study Solution: Shadow and G.U.N.}
    \begin{casesolution}{Shadow and G.U.N.\ Remote Access}
        \begin{enumerate}
            \footnotesize
            \item \textbf{Site-to-site VPN}: permanent IPsec tunnel between the HQ firewall and each office router.
            \item \textbf{Client-to-site VPN}: use a \textbf{full tunnel} because the laptops are on untrusted networks.
            \item \textbf{Clientless SSL VPN}: browser-only access to one web app with no software install.
        \end{enumerate}
    \end{casesolution}
    \vspace{0.2em}
    \begin{alertblock}{Key Lesson}
        \footnotesize Match the VPN type to who is connecting and how much access they need.
    \end{alertblock}
\end{frame}
...
\begin{frame}[t]{Secure Shell (SSH)}
    \begin{columns}[T]
        \begin{column}{0.50\textwidth}
            \begin{itemize}
                \item \keyterm{SSH} is the standard tool for secure remote \textbf{CLI access}.
                \item It runs on \textbf{TCP 22} and encrypts credentials and commands.
                \item Always use \textbf{SSH version 2}.
            \end{itemize}
            \vspace{0.3em}
            \begin{block}{Authentication Options}
                \begin{itemize}
                    \footnotesize
                    \item \keyterm{Password authentication}: simple, but weaker against brute force.
                    \item \keyterm{Key-based authentication}: stronger because the private key never leaves your device.
                \end{itemize}
            \end{block}
            \vspace{0.2em}
...
\begin{frame}[t]{Telnet: A Cautionary Tale}
    \begin{columns}[T]
        \begin{column}{0.50\textwidth}
            \begin{itemize}
                \item \keyterm{Telnet} was the original remote CLI protocol.
                \item It uses \textbf{TCP 23} and sends everything in \textbf{plaintext}.
                \item Any attacker on the path can capture usernames and passwords.
            \end{itemize}
            \vspace{0.3em}
            \begin{alertblock}{Never Use Telnet in Production}
                \footnotesize On real networks, disable Telnet and use SSH instead.
            \end{alertblock}
        \end{column}
        \begin{column}{0.47\textwidth}
            \begin{center}
...
\begin{frame}[t]{Remote Desktop Protocol (RDP)}
    \begin{columns}[T]
        \begin{column}{0.50\textwidth}
            \begin{itemize}
                \item \keyterm{RDP} provides a full \textbf{graphical desktop session} to a remote Windows system.
                \item It uses \textbf{TCP 3389} and encrypts traffic with TLS.
                \item Common uses:
                    \begin{itemize}
                        \footnotesize
                        \item Managing Windows servers.
                        \item Help-desk support or remote work access.
                    \end{itemize}
            \end{itemize}
            \vspace{0.3em}
            \begin{alertblock}{Critical Security Warning}
                \footnotesize Never expose RDP directly to the internet. Put it behind a VPN, firewall, or jump box.
            \end{alertblock}
        \end{column}
        \begin{column}{0.47\textwidth}
            \begin{center}
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Console Port Access}
                \begin{itemize}
                    \footnotesize
                    \item A \keyterm{console port} is a physical serial or USB management connection.
                    \item It gives CLI access that \textbf{bypasses the network entirely}.
                    \item Use it when IP settings are broken or remote access fails.
                \end{itemize}
            \end{block}
            \vspace{0.1em}
            \begin{block}{Out-of-Band (OOB) Management}
                \begin{itemize}
                    \footnotesize
                    \item \keyterm{OOB management} uses a \textbf{separate management network}.
                    \item If the production network fails, management access can still work.
                \end{itemize}
            \end{block}
        \end{column}
        \begin{column}{0.49\textwidth}
            \begin{center}
                \begin{tikzpicture}[scale=0.77, xshift=0.47cm]
                    % Core switch
            ...
            \begin{alertblock}{\footnotesize Sysadmin Saying}
                \footnotesize ``When SSH won't connect, the console port is your parachute.''
            \end{alertblock}
        \end{column}
    \end{columns}
\end{frame}
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Jump Box (Bastion Host)}
                \begin{itemize}
                    \footnotesize
                    \item A \keyterm{jump box} is a hardened server used as the single admin entry point.
                    \item Admins connect to it first, then move to internal devices.
                    \item It provides one audited entry path and keeps internal devices off the public edge.
                \end{itemize}
            \end{block}
        \end{column}
        \begin{column}{0.49\textwidth}
            \begin{block}{API-Based Management}
                \begin{itemize}
                    \footnotesize
                    \item Modern devices expose \keyterm{APIs} for automated management.
                    \item \keyterm{REST API}: HTTP plus JSON.
                    \item \keyterm{NETCONF}: XML over SSH with YANG models.
                    \item \keyterm{gRPC / gNMI}: common for streaming telemetry.
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
...
\begin{frame}[t]{Case Study: Dr.\ Eggman's Remote Management Crisis}
    \begin{casestudy}{Dr.\ Eggman's Remote Management Crisis}
        \footnotesize
        Orbot reports four urgent remote-management problems:
        ...
        \begin{center}
            \begin{tabular}{l p{8.0cm}}
                ...
                \midrule
                1 & Factory switches are unreachable over SSH because the production network is down. \\
                2 & A Windows server needs a GUI session for a critical patch. \\
                3 & 200+ devices need automated daily configuration backups. \\
                4 & An untrusted contractor needs browser-only access to one internal web portal. \\
                \bottomrule
            \end{tabular}
        \end{center}
    \end{casestudy}
    \begin{block}{Review Questions}
        \begin{enumerate}
            \footnotesize
            \item What should Eggman use when SSH is unavailable?
            \item Which tool fits the Windows server, automated backups, and contractor portal?
        \end{enumerate}
    \end{block}
\end{frame}
...
\begin{frame}[t]{Case Study Solution: Dr.\ Eggman's Remote Management}
    \begin{casesolution}{Dr.\ Eggman's Remote Management Crisis}
        \begin{enumerate}
            \footnotesize
            \item \textbf{Console port or OOB management}: use a path that does not depend on the production network.
            \item \textbf{RDP}: use it through a VPN or jump box, never directly from the internet.
            \item \textbf{REST API or NETCONF}: automate backups instead of doing manual SSH logins.
            \item \textbf{Clientless SSL VPN}: browser-only access scoped to one portal.
        \end{enumerate}
    \end{casesolution}
\end{frame}
