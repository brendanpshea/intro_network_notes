        ...
        \begin{column}{0.48\textwidth}
            \begin{block}{Topics Covered}
                \begin{enumerate}
                    ...
                    \item Modular Transceivers
                    \item Ethernet Frames \& MAC Addresses
                    \item Hubs, Bridges, and Switches
                    \item Switch Configuration
                    \item Switch Troubleshooting
                \end{enumerate}
            \end{block}
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{block}{Learning Outcomes}
                ...
                \begin{itemize}
                    \item Explain NIC functions and features
                    \item Interpret MAC address formats
                    \item Compare hubs, bridges, and switches
                    \item Configure basic switch settings
                    \item Troubleshoot common switch issues
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Hardware}
                \begin{itemize}
                    \item \keyterm{NIC} -- Network Interface Card
                    \item \keyterm{SFP/SFP+} -- Small Form-factor Pluggable
                    \item \keyterm{QSFP} -- Quad SFP (4 channels)
                    \item \keyterm{Transceiver} -- Converts signals
                \end{itemize}
            \end{block}
            ...
            \begin{block}{Addressing}
                \begin{itemize}
                    ...
                    \item \keyterm{OUI} -- Manufacturer identifier
                    \item \keyterm{Ethernet Frame} -- Data package
                \end{itemize}
            \end{block}
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{block}{Switching}
                \begin{itemize}
                    \item \keyterm{Hub} -- Repeats to all ports
                    \item \keyterm{Bridge} -- Learns MACs (2 ports)
                    \item \keyterm{Switch} -- Multi-port bridge
                    \item \keyterm{Managed/Unmanaged} -- Config options
                \end{itemize}
            \end{block}
            
            \vspace{0.3em}
            \begin{block}{Protocols \& Features}
                \begin{itemize}
                    \item \keyterm{STP} -- Spanning Tree Protocol
                    \item \keyterm{LACP} -- Link Aggregation
                    \item \keyterm{PoE} -- Power over Ethernet
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
...
% =============================================================================
% SLIDE 6: NIC Features and Specifications
% =============================================================================
...
% =============================================================================
% SLIDE 7: Modular Transceivers
% =============================================================================
\begin{frame}{Modular Transceivers: Flexibility Matters}
    \begin{columns}[T]
        \begin{column}{0.55\textwidth}
            \begin{itemize}
                \item Problem: Different cables need different ports---replacing switches is expensive!
                \item Solution: \keyterm{Modular transceivers}---swap the module, not the switch.
                \item \keyterm{SFP}: Small Form-factor Pluggable---up to 1 Gbps.
                \item \keyterm{SFP+}: Enhanced SFP---up to 10 Gbps.
                \item \keyterm{QSFP/QSFP+}: Quad SFP---40 to 100 Gbps using 4 channels.
            \end{itemize}
            
            \vspace{0.3em}
            \begin{exampleblock}{Why This Matters}
                Same switch can use copper OR fiber---just install the right transceiver module!
            \end{exampleblock}
        \end{column}
        \begin{column}{0.42\textwidth}
            \begin{center}
                \begin{tikzpicture}[scale=0.9]
                    % Switch
                    \draw[fill=rctcblue!20, line width=1pt, rounded corners] (0,0) rectangle (4,1.5);
                    \node[font=\small\bfseries] at (2, 1.2) {Switch};
                    
                    % SFP slots
                    \draw[fill=gray!30, line width=1pt] (0.3,0.3) rectangle (0.9,0.8);
                    \draw[fill=gray!30, line width=1pt] (1.1,0.3) rectangle (1.7,0.8);
                    \draw[fill=networkgreen!40, line width=1pt] (1.9,0.3) rectangle (2.5,0.8);
                    \draw[fill=fiberyellow!60, line width=1pt] (2.7,0.3) rectangle (3.3,0.8);
                    
                    % Labels
                    \node[font=\tiny] at (0.6, 0.5) {Empty};
                    \node[font=\tiny] at (2.2, 0.5) {Cu};
                    \node[font=\tiny] at (3.0, 0.5) {Fiber};
                    
                    % Transceiver being inserted
                    \draw[fill=aborange!50, line width=1pt] (1.2,1.8) rectangle (1.6,2.4);
                    \draw[-{Stealth}, line width=1pt] (1.4, 1.7) -- (1.4, 0.9);
                    \node[font=\tiny, above] at (1.4, 2.4) {SFP+};
                \end{tikzpicture}
            \end{center}
            
            \vspace{0.3em}
            \begin{block}{Transceiver Types}
                \small
                \begin{tabular}{l l}
                    \textbf{SFP-T} & Copper (RJ-45) \\
                    \textbf{SFP-SX} & MMF (short) \\
                    \textbf{SFP-LX} & SMF (long) \\
                \end{tabular}
            \end{block}
        \end{column}
    \end{columns}
    ...
    \begin{center}
        \begin{tikzpicture}[scale=0.9]
            ...
            \draw[fill=rctcblue!20, draw=rctcblue, line width=1.5pt, rounded corners] 
                (0,-0.6) rectangle (8,0.6);
            \node[font=\small\bfseries] at (4,-1) {Green Dragon Core Switch};
            
            % SFP+ slot (left side for server)
            \draw[fill=fiberyellow!60, line width=1pt] (0.3,-0.3) rectangle (0.8,0.3);
            \node[font=\tiny] at (0.55,-0.55) {SFP+};
            
            % RJ-45 ports (middle)
            \draw[fill=gray!30, line width=0.5pt] (2.8,-0.25) rectangle (3.2,0.25);
            \draw[fill=gray!30, line width=0.5pt] (3.8,-0.25) rectangle (4.2,0.25);
            \draw[fill=gray!30, line width=0.5pt] (4.8,-0.25) rectangle (5.2,0.25);
            \draw[fill=gray!30, line width=0.5pt] (5.8,-0.25) rectangle (6.2,0.25);
            \node[font=\tiny] at (4.5,-0.45) {RJ-45 Copper Ports};
            
            % SFP+ slot (right side for ISP)
            \draw[fill=fiberyellow!60, line width=1pt] (7.2,-0.3) rectangle (7.7,0.3);
            \node[font=\tiny] at (7.45,-0.45) {SFP+};
            
            % === FIBER CONNECTION TO SERVER (LEFT) ===
            \node[server] (srv) at (-2.5,0) {\iconserver[0.8cm]};
            \node[font=\small, below] at (-2.5,-0.6) {Cellar Server};
            
            % SFP+ transceiver at server
            \draw[fill=fiberyellow!60, line width=1pt] (-1.6,-0.2) rectangle (-1.2,0.2);
            \node[font=\tiny, above] at (-1.4,0.25) {SFP+};
            
            % Fiber cable (MMF - orange)
            \draw[fiber, line width=3pt] (-1.2,0) -- (0.3,0);
            \node[font=\scriptsize, fill=white, inner sep=2pt] at (-0.45,0) {MMF};
            
            % === COPPER CONNECTIONS TO DEVICES (TOP) ===
            % Front desk PC
            \node[pc] (pc1) at (3,3) {\iconpc[0.6cm]};
            \node[font=\small, above] at (3,3.5) {Front Desk};
            \draw[copper, line width=2pt] (3,0.6) -- (3,2.5);
            \node[font=\scriptsize, fill=white, inner sep=2pt] at (3,1.5) {Cat6};
            
            % Kitchen laptop
            \node[laptop] (pc2) at (5,3) {\iconlaptop[0.6cm]};
            \node[font=\small, above] at (5,3.5) {Kitchen};
            \draw[copper, line width=2pt] (5,0.6) -- (5,2.5);
            \node[font=\scriptsize, fill=white, inner sep=2pt] at (5,1.5) {Cat6};
            
            % Manager office
            \node[pc] (pc3) at (7,3) {\iconpc[0.6cm]};
            \node[font=\small, above] at (7,3.5) {Manager};
            \draw[copper, line width=2pt] (7,0.6) -- (7,2.5);
            \node[font=\scriptsize, fill=white, inner sep=2pt] at (7,1.5) {Cat5e};
            
            % === UPLINK TO ISP (RIGHT) ===
            \node[cloud] (cloud) at (10.5,0) {\iconcloud[0.8cm]};
            \node[font=\small, below] at (10.5,-0.6) {To Bree ISP};
            
            % SFP+ transceiver at ISP side
            \draw[fill=fiberyellow!60, line width=1pt] (9.2,-0.2) rectangle (9.6,0.2);
            \node[font=\tiny, above] at (9.4,0.25) {SFP+};
            
            % SMF to ISP (yellow for single-mode)
            \draw[draw=aborange, line width=3pt] (7.7,0) -- (9.2,0);
            \node[font=\scriptsize, fill=white, inner sep=2pt] at (8.45,0) {SMF};
            
            % === LEGEND ===
            \draw[copper, line width=3pt] (-2,-1.8) -- (-0.5,-1.8);
            \node[font=\scriptsize, right] at (-0.3,-1.8) {Copper (UTP)};
            
            \draw[fiber, line width=3pt] (2.5,-1.8) -- (4,-1.8);
            \node[font=\scriptsize, right] at (4.2,-1.8) {Multimode Fiber};
            
            \draw[draw=aborange, line width=3pt] (7,-1.8) -- (8.5,-1.8);
            \node[font=\scriptsize, right] at (8.7,-1.8) {Single-mode Fiber};
        \end{tikzpicture}
    \end{center}
    
    \vspace{0.3em}
    {\centering Layer 1 focuses on \textbf{physical connections}: cables, connectors, transceivers, and the signals they carry.\par}
\end{frame}
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{The Four Switch Actions}
                \begin{enumerate}
                    \item \keyterm{Learning}: See source MAC → record which port it came from
                    \item \keyterm{Forwarding}: Know destination MAC → send only to that port
                    \item \keyterm{Flooding}: Unknown destination → send to ALL ports (except source)
                    \item \keyterm{Filtering}: Same-segment traffic → don't forward
                \end{enumerate}
            \end{block}
...
% =============================================================================
\begin{frame}{Module 3.0 Summary}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Network Interfaces}
                \begin{itemize}
                    \item NICs connect computers to networks
                    \item MAC address: 48-bit hardware ID
                    \item OUI identifies manufacturer
                    \item SFP/SFP+ transceivers add flexibility
                \end{itemize}
            \end{block}
            
            \vspace{0.3em}
            \begin{block}{Switches}
                \begin{itemize}
                    \item Replaced hubs---dedicated bandwidth
                    \item Learn MACs, forward intelligently
                    \item Managed switches: VLANs, security
                    \item Layer 3 switches can route
                \end{itemize}
            \end{block}
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{block}{Configuration}
                \begin{itemize}
                    \item Link aggregation bundles links
                    \item MTU must match end-to-end
                    \item STP prevents loops automatically
                    \item PoE delivers power over Ethernet
                \end{itemize}
            \end{block}
            ...
            \begin{block}{Troubleshooting}
                \begin{itemize}
                    \item Check LEDs first---instant status
                    \item Use show commands for details
                    \item Watch error counters for trends
                    \item Beware loops and broadcast storms
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
