...
\usepackage[table]{xcolor} % adds rowcolor/column color support for tables
\usepackage{graphicx}
\usepackage{tikz}
\usepackage{booktabs}
\usepackage{array}
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Topics Covered}
                \begin{enumerate}
                    \item Internet Protocol Basics
                    \item IPv4 Address Format
                    \item Subnetting and CIDR
                    \item IP Troubleshooting Tools
                    \item IPv6 Addressing
                    \item IP Troubleshooting Scenarios
                \end{enumerate}
            \end{block}
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{block}{Learning Outcomes}
                ...
                \begin{itemize}
                    \item Explain Layer 2 vs Layer 3 addressing
                    \item Calculate subnet masks and host ranges
                    \item Configure IP addresses on devices
                    \item Use ping, arp, and ipconfig
                    \item Compare IPv4 and IPv6 formats
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
...
\begin{frame}{Key Terms Preview}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Addressing Concepts}
                \begin{itemize}
                    \item \keyterm{IP Address} -- Logical Layer 3 address
                    \item \keyterm{Subnet Mask} -- Defines network/host split
                    \item \keyterm{Default Gateway} -- Router to other networks
                    \item \keyterm{CIDR} -- Classless addressing notation
                \end{itemize}
            \end{block}
            ...
            \begin{block}{Protocols}
                \begin{itemize}
                    \item \keyterm{ARP} -- Resolves IP to MAC
                    \item \keyterm{ICMP} -- Ping and error messages
                    \item \keyterm{IPv4} -- 32-bit addresses
                    \item \keyterm{IPv6} -- 128-bit addresses
                \end{itemize}
            \end{block}
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{block}{Address Types}
                \begin{itemize}
                    \item \keyterm{Unicast} -- One-to-one
                    \item \keyterm{Broadcast} -- One-to-all (local)
                    \item \keyterm{Multicast} -- One-to-group
                    \item \keyterm{Anycast} -- One-to-nearest
                \end{itemize}
            \end{block}
            
            \vspace{0.3em}
            \begin{block}{Troubleshooting Tools}
                \begin{itemize}
                    \item \keyterm{ipconfig / ifconfig} -- View IP config
                    \item \keyterm{ping} -- Test connectivity
                    \item \keyterm{arp} -- View/manage ARP cache
                    \item \keyterm{traceroute} -- Show path to destination
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
...
% =============================================================================
\begin{frame}{Subnet Mask Examples}
    \begin{center}
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{itemize}
                ...
                \item If destination IP is NOT on your local subnet → send to the gateway.
...
\begin{frame}{Case Study Solution: Springfield Elementary Network Design}
    \begin{casesolution}{Springfield Elementary Network Design}
        \begin{enumerate}
            \item 28 hosts needs at least 30 usable addresses → \textbf{/27} (32 addresses, 30 usable).
    ...
    \begin{center}
        \begin{tikzpicture}[scale=0.9]
            ...
            \node[font=\tiny] at (2,0.15) {32 addresses → 30 usable};
            ...
            \node[font=\tiny] at (6,0.15) {16 addresses → 14 usable};
            ...
            \node[font=\tiny] at (9.1,0.15) {8 addr → 6 usable};
...
% =============================================================================
\begin{frame}{IPv6 Address Types Explained}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
...
% =============================================================================
\begin{frame}{IPv4 to IPv6 Transition}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
...
% =============================================================================
% SLIDE 37: Module 4.0 Summary - Part 1
% =============================================================================
\begin{frame}{Module 4.0 Summary: IPv4 Fundamentals}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{IP Basics}
                \begin{itemize}
                    \item IP operates at Layer 3
                    \item MAC for local, IP for end-to-end
                    \item ARP resolves IP to MAC
                    \item TTL prevents routing loops
                \end{itemize}
            \end{block}
            
            \vspace{0.3em}
            \begin{block}{IPv4 Addressing}
                \begin{itemize}
                    \item 32-bit addresses in dotted decimal
                    \item Subnet mask splits network/host
                    \item Gateway must be on same subnet
                    \item Private ranges for internal use
                \end{itemize}
            \end{block}
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{block}{Subnetting}
                \begin{itemize}
                    \item CIDR replaced classful addressing
                    \item VLSM allows variable subnet sizes
                    \item Usable hosts = $2^n - 2$
                    \item Network + broadcast reserved
                \end{itemize}
            \end{block}
            ...
            \begin{block}{Address Types}
                \begin{itemize}
                    ...
                    \item Broadcast -- one-to-all (local)
                    \item Multicast -- one-to-group
                    \item Anycast -- one-to-nearest
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
...
% =============================================================================
% SLIDE 38: Module 4.0 Summary - Part 2
% =============================================================================
\begin{frame}{Module 4.0 Summary: IPv6 and Troubleshooting}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Troubleshooting Tools}
                \begin{itemize}
                    \item ipconfig/ifconfig -- view config
                    \item ping -- test connectivity
                    \item arp -- view MAC mappings
                    \item Ping loopback $\rightarrow$ gateway $\rightarrow$ remote
                \end{itemize}
            \end{block}
            
            \vspace{0.3em}
            \begin{block}{IPv6 Addressing}
                \begin{itemize}
                    \item 128-bit addresses in hex notation
                    \item Simplify with :: and drop leading zeros
                    \item Link-local auto-configures (fe80::)
                    \item Global unicast (2000::/3)
                    \item Dual stack for transition
                \end{itemize}
            \end{block}
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{block}{Common Issues}
                \begin{itemize}
                    \item 169.254.x.x = DHCP failure
                    \item Empty ARP table = Layer 1/2 issue
                    \item Gateway wrong subnet = no routing
                    \item TTL expired = routing loop
                \end{itemize}
            \end{block}
            
            \vspace{0.3em}
            \begin{alertblock}{Key Takeaway}
                IP addressing is the foundation of all network communication. Master subnetting and the ping methodology -- both are on the exam!
            \end{alertblock}
        \end{column}
    \end{columns}
\end{frame}
