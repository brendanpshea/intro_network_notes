...
\section*{Review from Module 8}

\begin{frame}{Review: Operations and Monitoring Foundations}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Key Module 8 Ideas}
                \begin{itemize}
                    \item Documentation and change control reduce operational risk.
                    \item Monitoring data (SNMP, syslog, flow, packet capture) improves visibility.
                    \item Baselines and alerts help teams detect and respond to anomalies quickly.
                \end{itemize}
            \end{block}
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{alertblock}{Why This Matters in Module 9}
...
\begin{frame}[t,allowframebreaks]{Learning Outcomes}
    ...
    \small
    \begin{itemize}
        \item Explain the \keyterm{CIA Triad}: \keyterm{Confidentiality} (data secrecy), \keyterm{Integrity} (data accuracy), and \keyterm{Availability} (reliable access).
        \item Distinguish between \keyterm{authentication} (proving identity), \keyterm{authorization} (granting permissions), and \keyterm{accounting} (logging actions) in the AAA framework.
        \item Describe common threats: \keyterm{DoS/DDoS} (denial of service), \keyterm{malware} (viruses, worms, ransomware), and \keyterm{insider threats} (malicious or negligent employees).
        \item Explain \keyterm{social engineering} attacks including \keyterm{phishing} (fake emails), \keyterm{vishing} (voice phishing), \keyterm{smishing} (SMS phishing), and \keyterm{tailgating} (physical access).
        \item Describe \keyterm{spoofing} attacks: \keyterm{MAC spoofing} (impersonating devices), \keyterm{IP spoofing} (forging source addresses), \keyterm{ARP poisoning} (cache manipulation), and \keyterm{DNS poisoning} (name resolution hijacking).
        \item Identify rogue devices: \keyterm{rogue DHCP servers} (unauthorized IP assignment), \keyterm{rogue access points} (evil twins), and explain mitigation using DHCP snooping and 802.1X authentication.
        \item Explain password attacks: \keyterm{brute force} (trying all combinations), \keyterm{dictionary attacks} (common words), \keyterm{rainbow tables} (precomputed hashes), and defenses (complexity, length, salting, MFA).
        \item Describe \keyterm{Defense in Depth} using layered controls: physical security, firewalls, IDS/IPS, access control lists (ACLs), encryption, and security awareness training.
        \item Use \keyterm{Wireshark} to analyze packet captures and identify security indicators: DoS patterns, ARP poisoning, plaintext credentials, and suspicious traffic.
        \item Explain the role of \keyterm{penetration testing}, \keyterm{vulnerability scanning}, \keyterm{honeypots} (decoy systems), and \keyterm{SIEM} tools in proactive security operations.
    \end{itemize}
\end{frame}

% SLIDE 3: CIA Triad (Analogy Enhanced)
\section{Security Foundations}
\subsection{Core Principles and Controls}
        ...
        \begin{column}{0.48\textwidth}
            \begin{center}
                \begin{tikzpicture}[scale=0.8]
                    ...
                    \node[anchor=east, font=\tiny] at (-1.1, 0.55) {$\square$ Firewall Rules};
                    \node[anchor=east, font=\tiny] at (-1.1, 0.0) {$\square$ User Access};
                    \node[anchor=east, font=\tiny] at (-1.1, -0.55) {$\square$ Patch Levels};
...
\begin{frame}{Example: Threat Modeling a Small Network}
    \footnotesize
    Threat modeling is just structured guessing: what do we care about, who might attack it, and how could they do it?
    ...
    \begin{table}[]
        ...
        \begin{tabular}{|l|l|l|}
        \hline
        \rowcolor[HTML]{EFEFEF} \textbf{Asset} & \textbf{Likely Threat} & \textbf{Starter Control} \\ \hline
        Student accounts & Password reuse / phishing & MFA and password policy \\ \hline
        Switch ports & Rogue device or evil twin & Port security and 802.1X \\ \hline
        File server data & Ransomware or accidental deletion & Backups and least privilege \\ \hline
        Internet access & DNS or DHCP spoofing & DHCP snooping and DNS hygiene \\ \hline
        \end{tabular}
    \end{table}

    \begin{exampleblock}{Beginner takeaway}
        \footnotesize
        Start with your most valuable assets, then ask what could break confidentiality, integrity, or availability.
    \end{exampleblock}
\end{frame}

% SLIDE 8: Threat Assessment (Detail Enhanced)
\section{Threats and Risk Assessment}
\subsection{DoS, Botnets, and Malware}
...
\articleonly{
\par\small\emph{This insider threat diagram illustrates the asymmetry of internal versus external threats: a dashed box marks the Internal Network boundary with an insider user and server inside. A firewall blocks the external attacker (cat) from reaching the server (red arrow labeled "Firewall Blocks"). However, the insider (trusted employee icon) can reach the server directly via an orange arrow labeled "Direct Access," bypassing the firewall entirely. This visualization explains why insider threats are particularly dangerous: traditional perimeter security cannot defend against trusted users with legitimate access who have gone rogue or clicked a phishing link.}\par
}
...
\begin{frame}{Example: Security Controls Map}
    \footnotesize
    Beginners learn faster when a threat is paired with a control that actually reduces it.
    ...
    \begin{table}[]
        ...
        \begin{tabular}{|l|l|l|}
        \hline
        \rowcolor[HTML]{EFEFEF} \textbf{Threat} & \textbf{Defensive Control} & \textbf{Why it helps} \\ \hline
        Spoofed packets & ACLs and filtering & Limits what forged traffic can reach \\ \hline
        Password attacks & MFA and salting & Makes stolen or guessed passwords less useful \\ \hline
        Rogue DHCP server & DHCP snooping & Blocks unauthorized address assignment \\ \hline
        Ransomware & Backups and patching & Restores data and reduces infection paths \\ \hline
        Social engineering & Training and verification & Slows down human manipulation \\ \hline
        \end{tabular}
    \end{table}
\end{frame}
...
\articleonly{
\par\small\emph{This botnet zombie army diagram shows the scale of IoT compromise: a Command-and-Control server (cloud icon, top) issues dashed red commands to multiple zombie devices (hacked camera icons with semi-transparent appearance, showing they're under attacker control). Three green status dots below each camera indicate they're infected and listening to the C\&C. The three camera icons are typical targets: smart cameras, smart bulbs, or connected devices with weak passwords. Unlike traditional servers, IoT devices rarely receive timely security updates, making them attractive for long-term botnet recruitment.}\par
}
...
\begin{frame}{Case Study: The Zombie Accountants}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.25\textwidth}
            \begin{center}
                \begin{tikzpicture}[scale=0.65]
                    \node[draw, rounded corners, fill=yellow!20, align=center, minimum width=2.2cm] (alert) at (0, 1.4) {Dashboard\\Alert};
                    \node[draw, rounded corners, fill=red!10, align=center, minimum width=2cm] (infected) at (-1.0, -0.1) {Accounting\\PCs};
                    \node[draw, rounded corners, fill=green!10, align=center, minimum width=2cm] (action) at (1.0, -0.1) {Contain\\Port};
                    \draw[->, thick, alertred] (alert) -- (infected) node[midway, left, font=\tiny] {Spike};
                    \draw[->, thick, networkgreen] (alert) -- (action) node[midway, right, font=\tiny] {Isolate};
                    \node[font=\tiny, align=center] at (0, -1.6) {Thousands of requests is the clue.};
                \end{tikzpicture}
            \end{center}
        \end{column}
    \end{columns}
...
\section{Spoofing and On-Path Attacks}
\subsection{Identity and Traffic Manipulation}
        ...
        \begin{column}{0.42\textwidth}
            \begin{center}
                \begin{tikzpicture}[scale=0.7]
                    ...
                    \node[font=\tiny, fill=white, align=center] at (0, -1.0) {Traffic is relayed and can be modified in the middle.};
                \end{tikzpicture}
            \end{center}
        \end{column}
    \end{columns}
...
\begin{frame}{Case Study: The Coffee Shop Interception}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.25\textwidth}
            \begin{center}
                \begin{tikzpicture}[scale=0.7]
                    \node (user) at (-1.8, 0) {\icondog[0.6cm]};
                    \node[above, font=\tiny] at (-1.8, 0.45) {Snoopy};
                    \node (cat) at (0, 0) {\iconcat[0.7cm]};
                    \node[above, font=\tiny, text=alertred] at (0, 0.45) {ARP Poisoning};
                    \node (router) at (1.8, 0) {\iconrouter[0.6cm]};
                    \node[above, font=\tiny] at (1.8, 0.45) {Router};
                    \draw[->, thick, alertred] (user) -- (cat) node[midway, above, font=\tiny] {Traffic};
                    \draw[->, thick, alertred] (cat) -- (router) node[midway, above, font=\tiny] {Traffic};
                    \node[font=\tiny, fill=white, draw, align=center, text width=1.9cm] at (0, -1.0) {Certificate warning\\on-path interception};
                \end{tikzpicture}
            \end{center}
            \vspace{-0.2em}
            \centering\footnotesize\emph{A browser certificate warning is the clue that traffic may be going through someone else.}
        \end{column}
    \end{columns}
\end{frame}
...
\section{Rogue Services and Infrastructure Attacks}
\subsection{Rogue DHCP, DNS, and Access Points}
        ...
        \begin{column}{0.48\textwidth}
            \begin{center}
                \begin{tikzpicture}[scale=0.7]
                    ...
                    \node[above=0.25cm, font=\tiny, text=networkgreen] at (-2, 2.3) {Corp-WiFi (Real)};
                    
                    % Evil Twin
                    \node (fake) at (2, 2) {\faIcon{wifi}};
                    \node[above=0.25cm, font=\tiny, text=alertred] at (2, 2.3) {Corp-WiFi (Fake)};
                    ...
                    % Signal strength
                    \draw[->, dashed, thick, alertred] (fake) -- (vic) node[midway, right, font=\tiny, xshift=0.15cm] {Stronger Signal!};
                    \draw[->, dashed, gray] (real) -- (vic) node[midway, left, font=\tiny, xshift=-0.15cm] {Weaker};
                \end{tikzpicture}
            \end{center}
            \vspace{-0.2em}
            \centering\footnotesize\emph{The evil twin wins by copying the name and broadcasting more loudly.}
        \end{column}
    \end{columns}
\end{frame}
...
\section{Human-Centered Threats}
\subsection{Social Engineering and Credential Attacks}
...
\begin{frame}{Example: Phishing Email Breakdown}
    \footnotesize
    A phishing message usually works because it looks urgent and slightly familiar. Teach students to slow down and inspect it.

    \vspace{0.4em}
    \begin{exampleblock}{Fake message}
        \scriptsize
        From: it-support@kibblecorp-security.com

        Subject: Password Expiring Today

        Click here to keep your account active: \texttt{http://kibblecorp-login.example/renew}
    \end{exampleblock}

    \begin{alertblock}{Red flags to spot}
        \begin{itemize}
            \item Sender address is close, but not exact.
            \item The message creates urgency.
            \item The link uses plain HTTP and a suspicious domain.
        \end{itemize}
    \end{alertblock}
\end{frame}
...
% SLIDE 33: Recap
\section{Mitigation and Security Operations}
\subsection{Analysis, Defense in Depth, and Next Steps}
        ...
        \begin{column}{0.6\textwidth}
            \begin{center}
                \begin{tikzpicture}[scale=0.6]
                    ...
                    \node[left, font=\tiny, fill=white, draw] at (8.1, 0.5) {Found the SYN Flood!};
                \end{tikzpicture}
            \end{center}
            \vspace{-0.2em}
            \centering\footnotesize\emph{Repeated SYN packets from one source are a classic flood pattern in packet captures.}
        \end{column}
    \end{columns}
\end{frame}

\begin{frame}{Example: Incident Response Mini-Workflow}
    \footnotesize
    Security is not only about stopping attacks. It is also about what you do when something suspicious appears.

    \vspace{0.4em}
    \begin{enumerate}
        \item \textbf{Detect}: Notice the alert, log entry, or user report.
        \item \textbf{Contain}: Disconnect the rogue device or isolate the host.
        \item \textbf{Investigate}: Check logs, packet captures, and affected accounts.
        \item \textbf{Recover}: Restore services from clean backups and re-enable access carefully.
        \item \textbf{Improve}: Document the cause and update controls so it is less likely to happen again.
    \end{enumerate}

    \begin{exampleblock}{Simple classroom example}
        \footnotesize
        If a rogue DHCP server is discovered, the first move is containment: unplug the device or shut down the port, then verify clients received correct addresses.
    \end{exampleblock}
\end{frame}
...
\begin{frame}[t,allowframebreaks]{Module 9.0 Summary}
    \small
    \textbf{Key Concepts:}
    \begin{itemize}
        \item \keyterm{CIA Triad}: \keyterm{Confidentiality} (encryption, access control), \keyterm{Integrity} (hashing, digital signatures), \keyterm{Availability} (redundancy, DDoS mitigation).
        \item \keyterm{AAA}: \keyterm{Authentication} (who are you?), \keyterm{Authorization} (what can you do?), \keyterm{Accounting} (what did you do?). Use TACACS+ or RADIUS.
        \item \keyterm{DoS/DDoS}: Denial of Service attacks flood targets with traffic. Mitigate with rate limiting, traffic filtering, and CDN protection.
        \item \keyterm{Malware types}: Viruses (attach to files), Worms (self-replicate), Trojans (disguised), Ransomware (encrypt files for payment). Use antivirus, patching, and backups.
        \item \keyterm{Social engineering}: Human-based attacks. \keyterm{Phishing} (fake emails), \keyterm{Vishing} (voice calls), \keyterm{Smishing} (SMS), \keyterm{Tailgating} (physical access). Defense: training and awareness.
        \item \keyterm{Spoofing}: \keyterm{MAC spoofing} (change NIC address), \keyterm{IP spoofing} (forge source IP), \keyterm{ARP poisoning} (man-in-the-middle), \keyterm{DNS poisoning} (redirect to malicious sites).
        \item Rogue devices: \keyterm{Rogue DHCP} (unauthorized IP assignment, VLAN hopping). \keyterm{Rogue AP} (evil twin). Mitigate with DHCP snooping, 802.1X (port-based NAC), and wireless IDS.
        \item Password attacks: \keyterm{Brute force} (all combinations), \keyterm{Dictionary} (common words), \keyterm{Rainbow tables} (precomputed hashes). Defense: complexity (8+ chars, mixed case, symbols), \keyterm{salting}, \keyterm{MFA}.
        \item \keyterm{Defense in Depth}: Layered security. Physical security, network segmentation, firewalls, \keyterm{IDS} (detect), \keyterm{IPS} (prevent), ACLs, encryption (TLS, VPN), security training.
        \item \keyterm{Wireshark}: Packet capture for forensics. Identify DoS floods, ARP poisoning (duplicate MACs), plaintext passwords (HTTP, Telnet). Filter by protocol, IP, port.
    \end{itemize}
    \framebreak
    \vspace{0.3em}
    \textbf{Security Operations:}
    \begin{itemize}
        \item \keyterm{Penetration testing}: Authorized simulated attacks (red team) to find vulnerabilities before attackers do.
        \item \keyterm{Vulnerability scanning}: Automated tool (Nessus, OpenVAS) identifying known CVEs and misconfigurations.
        \item \keyterm{Honeypots}: Decoy systems to lure attackers, gather intelligence, and distract from production assets.
        \item \keyterm{SIEM}: Security Information and Event Management. Aggregate logs, correlate events, generate alerts. Examples: Splunk, QRadar.
        \item Security policies: Acceptable Use Policy (AUP), Incident Response Plan, Password Policy, Data Classification, Physical Access Controls.
    \end{itemize}
\end{frame}
...
\begin{frame}{Beginner Checklist: What to Turn On First}
    \footnotesize
    If you are securing a small lab or home office, start with controls that give the biggest benefit for the least complexity.
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Highest Priority}
                \begin{itemize}
                    \item MFA for accounts that can reach sensitive systems.
                    \item Automatic patching for endpoints and servers.
                    \item Backups you have actually tested.
                \end{itemize}
            \end{block}
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{block}{Next Layer}
                \begin{itemize}
                    \item Least privilege for users and admins.
                    \item Logging and alerting on important systems.
                    \item DHCP snooping, port security, and basic ACLs on the network edge.
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
