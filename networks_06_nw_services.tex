...
% Theme: Batman Universe Characters
% =============================================================================
...
\tikzstyle{portbox} = [draw=tcpblue, fill=tcpblue!10, rounded corners,
                       minimum height=0.4cm, font=\small]
\tikzstyle{dnsbox} = [draw=dnsyellow, fill=dnsyellow!15, rounded corners,
                      minimum height=0.4cm, font=\small]
...
\newcommand{\iconfirewall}[1][0.6cm]{\resizebox{#1}{!}{\faShieldAlt}}
...
\begin{frame}{Key Terms Preview}
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Transport Layer}
                \begin{itemize}\setlength{\itemsep}{1pt}
                    \item \keyterm{TCP} -- Reliable, connection-oriented
                    \item \keyterm{UDP} -- Fast, connectionless
                    \item \keyterm{Port} -- Service identifier (0--65535)
                    \item \keyterm{Socket} -- IP address + port combo
                \end{itemize}
            \end{block}
            
            \begin{block}{DHCP Concepts}
                \begin{itemize}\setlength{\itemsep}{1pt}
                    \item \keyterm{DORA} -- Discover, Offer, Request, Ack
                    \item \keyterm{Lease} -- Temporary IP assignment
                    \item \keyterm{Scope} -- Range of IPs to assign
                    \item \keyterm{Relay} -- Forwards DHCP across subnets
                \end{itemize}
            \end{block}
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{block}{Auto-Configuration}
                \begin{itemize}\setlength{\itemsep}{1pt}
                    \item \keyterm{APIPA} -- 169.254.x.x fallback
                    \item \keyterm{SLAAC} -- IPv6 self-configuration
                    \item \keyterm{EUI-64} -- MAC-based IPv6 address
                    \item \keyterm{DHCPv6} -- IPv6 address assignment
                \end{itemize}
            \end{block}
            
            \begin{block}{DNS Concepts}
                \begin{itemize}\setlength{\itemsep}{1pt}
                    \item \keyterm{FQDN} -- Full domain name (host.domain.tld)
                    \item \keyterm{Zone} -- Administrative DNS region
                    \item \keyterm{Record} -- DNS entry (A, AAAA, MX, etc.)
                    \item \keyterm{Resolver} -- DNS lookup service
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
...
\begin{frame}{Transport Layer and Ports}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            ...
            \vspace{0.2em}
            \begin{block}{Port Ranges}
                \small
                \begin{tabular}{l l}
                    \toprule
                    \textbf{Range} & \textbf{Usage} \\
                    \midrule
        ...
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{block}{TCP/UDP Segment Structure}
                \centering
                \small
                \setlength{\tabcolsep}{2pt}
                \begin{tabular}{|>{\columncolor{tcpblue!20}}c|>{\columncolor{tcpblue!20}}c|}
                    \hline
                    \textbf{Source Port} & \textbf{Dest Port} \\
                    \footnotesize 16 bits & \footnotesize 16 bits \\
                    \hline
                    \multicolumn{2}{|>{\columncolor{rctcblue!10}}c|}{Other Header Fields} \\
                    \hline
            ...
            \vspace{0.3em}
            \begin{block}{Socket Example}
                \centering
                \texttt{\colorbox{rctcblue!15}{192.168.1.10}:\colorbox{tcpblue!20}{443}}
                
                \vspace{0.2em}
                \small
                \textcolor{rctcblue}{IP Address} + \textcolor{tcpblue}{Port} = Socket
            \end{block}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.48\textwidth}
            \begin{exampleblock}{Memory Tip}
                \small
                Think: ``\textbf{S}end → \textbf{S}end back with \textbf{A}ck → \textbf{A}cknowledge''
            \end{exampleblock}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Case Study Solution: Batman \& Oracle}
    \begin{casesolution}{The Mission Communications Problem}
        \small
        \begin{enumerate}\setlength{\itemsep}{1pt}
            \item \textbf{Video files → TCP}: Missing frames corrupt evidence. TCP guarantees delivery.
            \item \textbf{Voice comms → UDP}: Real-time essential. Brief glitches beat lag. UDP wins.
...
\begin{frame}{DHCP DORA Process}
    \begin{center}
        \begin{tikzpicture}[scale=0.75]
            ...
            \node[font=\tiny, dhcppurple] at (4.5,2.2) {Broadcast: ``Anyone have an IP?''};
        ...
        \begin{column}{0.48\textwidth}
            \begin{center}
                \begin{tikzpicture}[scale=0.6]
                    ...
                    \node[font=\tiny, networkgreen, align=center] at (2.5,2.3) {Router\\Advertisement};
                    \node[font=\tiny, networkgreen] at (2.5,1.5) {Prefix: 2001:db8::/64};
                    ...
                    % Labels
                    \node[font=\tiny, rctcblue] at (0.5,-0.8) {Prefix};
                    \node[font=\tiny, aborange] at (4.5,-0.8) {EUI-64};
                \end{tikzpicture}
            \end{center}
        ...
        \begin{column}{0.48\textwidth}
            \begin{block}{Resolution Steps}
                ...
                \begin{enumerate}\setlength{\itemsep}{0pt}
                    ...
                    \item Resolver asks root → ``Try .com''
                    \item Resolver asks .com → ``Try google.com NS''
                    \item Resolver asks google.com → IP!
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.48\textwidth}
            \begin{block}{CNAME Record (Alias)}
                ...
                Use cases:
                \begin{itemize}\setlength{\itemsep}{1pt}
                    \item Multiple names → same server
                    \item CDN redirects
...
\begin{frame}{DNS Records: MX, SRV, TXT, PTR}
    ...
    \begin{block}{Record Types}
        \begin{tabular}{l l l}
            ...
            \textbf{PTR} & Reverse lookup (IP→name) & \texttt{34.216.184.93.in-addr.arpa. PTR www.example.com.} \\
            \bottomrule
        \end{tabular}
    \end{block}
...
\begin{frame}{DNS Troubleshooting Tools}
    ...
    \begin{exampleblock}{Troubleshooting Steps}
        \small
        1. Check local DNS settings (\texttt{ipconfig /all}) → 
        2. Query local resolver → 
        3. Query external DNS (8.8.8.8) → 
        4. Compare results
    \end{exampleblock}
\end{frame}
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Transport Layer}
                \small
                \begin{itemize}\setlength{\itemsep}{1pt}
                    \item \textbf{TCP}: Reliable, connection-oriented
                    \item \textbf{UDP}: Fast, connectionless
                    \item \textbf{Ports}: 0--65535, identify services
                    \item \textbf{netstat}: View connections
                \end{itemize}
            \end{block}
            ...
            \begin{block}{DHCP}
                \small
                \begin{itemize}\setlength{\itemsep}{1pt}
                    \item \textbf{DORA}: Discover, Offer, Request, Ack
                    \item \textbf{Scope}: IP address pool
                    \item \textbf{Relay}: Forward across subnets
                    \item \textbf{APIPA}: 169.254.x.x fallback
                \end{itemize}
            \end{block}
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{block}{IPv6 Auto-Config}
                \small
                \begin{itemize}\setlength{\itemsep}{1pt}
                    \item \textbf{SLAAC}: Stateless, uses RA + EUI-64
                    \item \textbf{DHCPv6}: Stateful or stateless modes
                \end{itemize}
            \end{block}
            ...
            \begin{block}{DNS}
                \small
                \begin{itemize}\setlength{\itemsep}{1pt}
                    \item Hierarchical: Root → TLD → Domain
                    \item Records: A, AAAA, CNAME, MX, PTR
                    \item Tools: nslookup, dig
                    \item Security: Cache poisoning risk
                \end{itemize}
            \end{block}
            
            \vspace{0.2em}
            \begin{exampleblock}{Next Module}
                \small
